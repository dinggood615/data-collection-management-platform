...
添加 `https://www.cnpcbidding.com/#/tenders` 后，平台会自动选用“中石油专用动态浏览器”。该站是 JavaScript 动态表格，并可能显示网站验证弹框：

1. 点击“打开此站验证”，在可视 Chrome 中手动完成网站允许的验证并停留在公告列表。
2. 返回平台点击“重新识别”。系统识别到公告表格后会自动切换为“已适配（动态浏览器）”并启用。
3. 定时任务复用持久化会话，按照目标日期受控翻页；会话失效时会停止该站点并提示重新验证。

专用适配器不会破解验证码、绕过访问控制，也不会复刻网站的加密接口。
