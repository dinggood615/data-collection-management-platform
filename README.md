...
自定义站点的采集规则失效后，平台会自动重新分析页面结构、公开数据接口和已有浏览器会话，保存恢复后的规则并在当轮立即重试。短暂网络故障不会自动停用站点；恢复尚未成功时会在后续采集继续尝试，无需在网页执行人工验证。

性能策略针对小型 VPS 优化：静态页面与公开接口默认最多 3 路并发，动态浏览器保持单路以控制内存；历史补录只重复运行支持日期查询的公开接口和专用采集器；同一站点每轮最多重建一次规则。可通过 `CUSTOM_SITE_WORKERS=1..4` 调整静态采集并发数。
...
- 默认 1 个 CPU 线程、2048 上下文、1050MB 地址空间上限、90 秒超时，每批最多 3 项；实测模型峰值物理内存约 603MB。
...
Docker 版提供静态站点采集、数据库、邮件、企业微信、备份迁移和恢复功能；不会安装宿主机可视 Chrome/noVNC。需要浏览器渲染的动态站点建议使用原生 Linux 版本，以便自动恢复流程复用浏览器会话。
...
6. 如果提示“待自动恢复”，保持站点启用即可；采集任务会自动重建规则并重试，也可以点击“立即自动重识别”。
...
原生安装会运行持久浏览器服务。动态站点规则失效后，平台先尝试静态结构和公开数据接口，再自动复用已有浏览器会话识别页面；成功后保存新规则并在当轮重试。若目标网站明确要求登录、验证码或访问授权，平台会保留旧规则并在后续任务继续尝试，但不会破解或绕过网站的安全机制。

### 中石油招标网

添加 `https://www.cnpcbidding.com/#/tenders` 后，平台会自动选用“中石油专用动态浏览器”。适配器复用可用会话读取页面公开呈现的 Vue 公告数据，并调用页面自身的翻页和详情操作，因此不依赖传统链接或表格选择器。会话失效时自动进入恢复流程；网站明确要求验证码或登录时不会尝试绕过。
